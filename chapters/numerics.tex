...
\section{Numerical Error and Computation Time}
    ...
    \subsection{Full stream generation}
        ...
        In this section, I combine these two components: I examine the quality of orbit integration for star particles evolving within a globular cluster, which in turn is orbiting in the Galactic potential.
        ...
        Fig.~\ref{fig:NumericalErrorStreamRetrace_NGC6171} shows the results of this experiment across four different timesteps. As expected, increasing the timestep degrades time-reversibility. As expected, the timestep of $\alpha\approx1$ (top left), does not retrace the orbit at all. Notably, the retrace still preserves the form of a stream and does not recoallesce into a cluster. However, the situation improves quickly. Already at $\alpha\approx 2\times10^{-1}$, the star-particles almost return to their departure point with just a slight offset. At $\alpha\approx 2\times10^{-2}$, most of the particles return exactly to their intial conditions, barring a few. At $\alpha\approx 6\times10^{-3}$, the retrace is equisite.

        \begin{figure}
            \centering
            \includegraphics[width=\linewidth]{images/NumericalErrorStreamRetrace_NGC6171.png}
            \caption{Time-reversibility test for NGC~6171. The cluster's orbit was integrated backward by 1 Gyr, and a Plummer sphere of 512 particles was initialized at that position (``Initial''). The system was then integrated forward (``Stream'') and backward again (``Retrace''). Each panel shows a different timestep, which was selected as a fraction of the internal dynamical time $\tau$. The sub-panels are zooms of the globular cluster, comparing the initial conditions to the retrace. The quality increases from top to bottom, as the timestep decreases.}
            \label{fig:NumericalErrorStreamRetrace_NGC6171}
        \end{figure}

        Fig.~\ref{fig:NumericalErrorStreamRetrace_EnergyErrors} shows the integrators conservation of energy while retracing to initial conditions from Fig.~\ref{fig:NumericalErrorStreamRetrace_NGC6171} for the four different investigated timesteps. 

        \begin{figure}
            \centering
            \includegraphics[width=\linewidth]{images/NumericalErrorStreamRetrace_EnergyErrors.png}
            \caption{The relative error in the energy conservation of the retrace compared to the initial conditions. As expected, the quality improves with decreasing timestep. The \textit{stream} shows each particles distribution in energy at the final position compared to the initial position, which need not necessarily be conseved. Indeed, this is in essence showcasing the spread in initial orbital energies relative to the cluster's center of mass. Increasing the timestep tends toward this distribution.}
            \label{fig:NumericalErrorStreamRetrace_EnergyErrors}
        \end{figure}

        By inspecting the largest timestep presented in Fig.~\ref{fig:NumericalErrorStreamRetrace_NGC6171}, we observe the that the solution is erroneous. However, Fig.~\ref{fig:NumericalErrorStreamRetrace_EnergyErrors} shows that the error in the energy conservation is between $10^{-3}$-$10^{-2}$. However, we must interprete this with caution. We are actually witnessing the spread in orbital energies of the star particles within the cluster compared to the orbital energy of the center of mass, which is acting as a ceiling for this metric. To show this point, we can make a rough estimate of this ratio by comparing the internal potential energy scale of the cluster to its orbital energy in the Galaxy. The characteristic internal potential is on the order of $\Phi \sim GM/a$, where $M \sim 10^5~M_\odot$ and $a \sim 0.005$~kpc, yielding $\Phi \sim 10^2~\mathrm{km}^2\,\mathrm{s}^{-2}$. In contrast, the typical orbital energy of a globular cluster in the Milky Way is about $10^5~\mathrm{km}^2\,\mathrm{s}^{-2}$ (see Fig.~\ref{fig:energy_sensitivity_analysis_MWGCS_to_distance_RV_mu}). 
