...
\section{Solving the equations of motion}
    ...
    Euler proposed a simple method for numerically solving differential equations far before the advent of computers. However, this can be quite complex for systems of coupled equations. Additionally, since we are solving Hamilton's equations, there are spetial properties that can be exploited to create a numerical schema that gives a numerical solution that is consistent to the true solution. 

    In this thesis, I implemented the Leapfrog integrator, as well as the Ruth-Ford schema. These schema were created from using properties of hamiltonian mechanics and are called \textit{sympletic} schemas. Before continuing, I would like to quote \citep{bovy_inprep}:
    ...
    However, my curiosity in linguistics pushed me to delve further. What does \textit{sympletic} mean? The word was invented by \citep{weyl1946classical} because \textit{complex} was already taken. The prefix \textit{com} refers to together, and "plexus" comes from greek meaning ``woven'' or braided. So complex, from its roots, means when multiple things are inter-woven or interlaced. Tangled braids are indeed complicated to the imagery works. \textit{Sym} is the greek prefix for together. But it communicates the same thing, the components evolution of the position and momentum depend upon one another. The woveness of hamiltonian dynamics is clear how $\dot{p} = -\frac{\partial \mathcal{H}}{\partial q}$ and $\dot{q} = \frac{\partial \mathcal{H}}{\partial p}$. 
