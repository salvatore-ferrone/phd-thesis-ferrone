...
\section{Numerical Error and Computation Time}
    ...
    \subsection{Star-particles in a static globular cluster}
        ...
        Now, what is a good value of $\alpha$ to ensure energy conservation? From the analysis presented in the previous sections, I choose timestamps without relating them to the crossing times of the globular clusters' orbits. Fig.~\ref{fig:numericalErrorLeapfrogVanilla} shows that $\Delta t = 10^4~\mathrm{yr}$ achieves a mean relative energy error of $10^{-8}$. Meanwhile, Fig.~\ref{fig:numericalErrorReverseIntegration} shows that a timestep of $\Delta t = 10^5~\mathrm{yr}$ already achieves convergence in time-reversibility. If we relate these numbers to the mean crossing time from Fig.~\ref{fig:GCsystemCharacteristicTimes}, we obtain an $\alpha$ of about $2 \times 10^{-4}$ and $2 \times 10^{-3}$, respectively. In Fig.~\ref{fig:numericalErrorStaticPlummerSphereEnergyError} I investigate the energy conservation of a normalized plummer sphere with trial $\alpha$'s of $[10^{-4},10^{-3},10^{-2},10^{-1},1,2]$. This experiment shows that $\alpha$ below $10^{-2}$ has excellent energy conservation. The experiment compares each star-particle's final energy to it's initial energy. In this section, we do not study the time-reversability, but present it for simulating the full disruption in the following section. 
        \begin{figure}
            \centering
            \includegraphics[width=\linewidth]{images/numericalErrorStaticPlummerSphereEnergyError.png}
            \caption{The relative error of the energy conservation for a plummer sphere with different timesteps. Each sphere was sampled with a the same number of particles, $N_p$, and integrated for the same amount of time, $T$, both of which are indicated on the plot. The x-axis is the size of the time-step, $\alpha$, which is the fraction of the internal dynamical time, $\tau = \sqrt{\frac{a^3}{GM}}$.}
