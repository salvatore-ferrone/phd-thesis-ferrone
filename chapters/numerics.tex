...
\section{Numerical Error and Computation Time}
    ...
    \subsection{Star-particles in a static globular cluster}
        ...
        To properly compute the orbits of the star-particles, the timestep must be small enough to properly resolve the orbit while it is inside the cluster. How should we pick that number? There are two criteria, the first is to ensure that the timestep is some fraction of the cluster's dynamical time:
        \begin{equation}
            \Delta t' = \alpha ' \tau.
        \end{equation}
        I mark each with a \textit{prime} becuase they are a trial time-step until the second criteria is also satisfied, which is simply that the resultant number of timesteps is an integer: $ N = \lceil T / \alpha ' \tau \rceil$, where $T$ is the total integration time. We round-up the number of steps to choose an even smaller timestep, which becomes: $\Delta t = T/N$, which also adjusts: $\alpha = \tau / \Delta t$. 


        The way I present the experiments in this section, is that I select some fractions of each cluster's dynamical time, and explore the resultant numerical error. I wanted to test a wide range of $\alpha$. To do this, I only pick few different trial $\alpha '$, and then round each one down so that that the number of steps is not only an integer, but also a multiple of 2. This leads to the following criterion:
        \begin{equation}
            N = 2^{k-1}
        \end{equation}
        where $k$ is an integer index $ k \in [1,2,\dots]$. The criterion for the number of steps becomes: 
        \begin{equation}
