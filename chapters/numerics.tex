...
\section{Numerical Error \& Computation Time}
    ...
    \subsection{Full stream generation}
        ...
        Why is the energy change bounded around $10^{-1}$? Consider a thought experiment: if the cluster were removed and only the star particles remained, freely orbiting in the Galactic potential. The characteristic energy scale of the cluster is much smaller than the orbital energy in the Galaxy. Using $\Phi = GM/a$ with $M \sim 10^5~M_\odot$ and $a \sim 0.005$~kpc yields an internal energy scale on the order of $10^2~\mathrm{km}^2\,\mathrm{s}^{-2}$, while Fig.~\ref{fig:energy_sensitivity_analysis_MWGCS_to_distance_RV_mu} shows that typical orbital energies are around $10^5~\mathrm{km}^2\,\mathrm{s}^{-2}$. This discrepancy of roughly three orders of magnitude explains why internal energy changes appear small in absolute terms, but relatively large when considered as a fraction of the (small) cluster energy scale.
