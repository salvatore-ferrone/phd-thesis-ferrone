...
\section{Numerical Error \& Computation Time}
    ...
    \subsection{Full stream generation}
        ...
        \begin{figure}
            ...
            \caption{The center of mass of NGC6171 was integrated back in time by 1~$\mathrm{s}~\frac{\mathrm{kpc}}{\mathrm{km}}$. A plummer sphere was sampled with 512 particles an placed about this position and is labeled ``Initial'' in the figure. This sphere was integrated forward to create the ``Stream''. The relative error of the conservation of energy is distributed in the right panel. The stream was then integrated backward in time by 1~$\mathrm{s}~\frac{\mathrm{kpc}}{\mathrm{km}}$ and is labeled with ``Retrace''. The time step was selected as a fraction of the internal dynamical time scale, $\tau$. }
        ...
        \begin{figure}
            ...
            \caption{The same as Fig.~\ref{fig:NumericalErrorStreamRetrace_NGC6171_Nsteps_524288_stepsPerTau_155} but with larger timesteps, as indicated on the panel.}
            \label{fig:NumericalErrorStreamRetrace_NGC6171_Nsteps_4096_stepsPerTau_1}
        \end{figure}

        Both of the time steps in Fig.~\ref{fig:NumericalErrorStreamRetrace_NGC6171_Nsteps_524288_stepsPerTau_155} sufficiently retrace the cluster's initial conditions to a satirsfactory degree. I compare the change in energy of each particle from it's initial conditions at both the final simulation time and the retrace. For $\Delta t \sim 6\times 10^{-3}\tau$, the majority of particles conserve energy in their retrace between $10^{-7}-10^{-4}$, while $\Delta t \sim 3\times 10^{-2}\tau$ is shifted upward by about an order of magnitude. 

        Note that for each of the distributions of conservation of energy at the final timestep, they are between $10^{-4}-10^{-1}$, and particularly how this distribution is independent of the chosen timestep. First, the fact that the energy is not well conserved is expected because the equations of motion do not preserve energy, as the hamiltonian time dependent. Secondly, the reason that difference in energy is nonetheless less than $10^{-1}$ and doesn't dramatically change is that for each of the investigated timesteps, it is still more than sufficient for integrated orbits within the galaxy. Indeed, from Fig.~\ref{fig:numericalErrorLeapFrogVanilla} and Fig.~\ref{fig:numericalErrorRuthForest}, we note that a timestep of $10^{5}$~years is more than sufficient for properly integrating the equations of motion. Indeed, even for the bottom panel of Fig.~\ref{fig:NumericalErrorStreamRetrace_NGC6171_Nsteps_4096_stepsPerTau_1}, a time step equal to the internal dynamical for NGC~6171 is still $10^{5}$~years. Thus, the magnitude of the conservation of energy here is really due to the fact that the energy is not well conserved, but its only going to vary so much. How much could it vary? Basically, if the cluster didn't exist, but there was still a population of stars floating through the galaxy, how much would we expect? Well, we can say that the energy varys on the order of the interal energy of a cluster and the total energy is the orbital energy that we'd expect for someone orbting the galaxy. For example, just to get a quick estiamte: $\Phi=GM/a$, and taking typical values for the mass as $10^{5}~\mathrm{M}_\odot$ and $0.005~kpc$, the energy of the cluster is on the order of $10^{2}~\frac{\mathrm{km}^2}{\mathrm{s}^2}$, and from Fig.~\ref{fig:energy_sensitivity_analysis_MWGCS_to_distance_RV_mu}, the orbital energies are on the order of $10^{5}~\frac{\mathrm{km}^2}{\mathrm{s}^2}$. This difference perfectly explains what observed resultant distribution. 


        Ok, so that was a good singular example, what about the whole catalog? Fig.~\ref{fig:NumericalErrorStreamRetraceEnergyConservation} shows the whole catalog.
