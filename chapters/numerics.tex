...
\section{Numerical Error \& Computation Time}
    ...
    \subsection{Star-particles in the galaxy}
        ...
        \end{figure}
        Interestingly, the energy conservation for the same timestep is much worse than a plummer sphere in isolation. By inspecting Fig.~\ref{fig:numericalErrorStaticPlummerSphereEnergyError}, we would expect a median relative error on the order of $10^{-8}$, yet in Fig.~\ref{fig:NumericalErrorStreamRetrace_Pal5_Nsteps_32768_stepsPerTau_420}, we observe an error on the order of $10^{-4}$.
