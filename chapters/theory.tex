...
\section{The Ignored Physics} \label{sec:ignoredphysics}
    ...
    \subsection{Globular Cluster Internal Dynamics} \label{sec:collisionalDynamics}
        ...
        \subsubsection{Beyond Two-Body Relaxation}
            % Two-body relaxation—is not the only process at play within globular clusters. In fact, the full range of dynamical interactions in such systems is vast and complex. A careful reduction of this complexity into a few key mechanisms is the aim of the authoritative textbook by \citet{2003gmbp.book.....H}. Two-body relaxation is one example of \textit{micro-physical} process within the globular clusters, specifically it is the cummulative sum of many two body interactions.

            % Another example of a microphysical process within clusters are \textit{three-body encounters}. This occurs when one star interacts with a binary system. Recent studies by \citet{2023MNRAS.518.4249G} and \citet{2024MNRAS.528.5189G} have focused on the role of three-body encounters in globular clusters. In these interactions, a single star can extract energy from a binary system—sometimes enough to be ejected from the cluster entirely. This is a fundamentally different mechanism from the tidal stripping investigated in this thesis. Tidal fields gently transfer energy to stars, allowing them to escape through the Lagrange points. In contrast, three-body encounters can impart a significant energy boost to a star, potentially enabling it to escape the cluster altogether—even from regions that are otherwise forbidden, as indicated by Fig.~\ref{fig:CR3BP_forbidden_region}.

            % In \citet{2024MNRAS.528.5189G}, similarly to our work in Chapter~4, the authors investigated mass loss from the Milky Way globular clusters. However, rather than focusing on external tidal forces, they examined internal three-body encounters occurring in the cluster cores. One of their striking results is reproduced in Fig.~\ref{fig:grondin_et_al_core_spray}, which clearly shows that many stars are ejected onto orbits that no longer align with the parent cluster—effectively spreading across the entire sky.
            
            % Not only does a new world of physics emerge when we consider stars to have mass, but a lot of interesting physics opens up once we consider them to have different masses. For instance, there is a similar process to two-body relaxation which is \textit{dynamical friction} \citep{1943ApJ....97..255C,1943ApJ....97..263C,1943ApJ....98...54C}.
            
            % In essence, if a body is flying through a medium and if it is more massive than the other bodies, then it will tend to \textit{lose} energy and fall toward the center of the system. When considering a population within a globular cluster, this effect at scale leads to \textit{mass-segregation}, where massive stars populate lower energy orbits while the lower mass stars populate higher energy orbits. Mass egregation is well known and is presented in a variety of works that simulate the internal dynamics of globular clusters \citep{1997A&ARv...8....1M,2002ApJ...570..171F,2016MNRAS.458.1450W}.

            % Stars, interestingly, are not just point masses that exert gravitational forces. Indeed, they are super-hot gaseous spheres in hydrostatic equilibrium, balancing their own gravitational collapse with an outward pressure sustained by thermonuclear fushion \citep{2013sse..book.....K}. Also, stars evolve with time. \citep{2023LRCA....9....3S}
            
            % \citet{2018MNRAS.474.2479B} consider how stellar evolution plays a role in stellar streams. What if you have mass segregation and subsequently eject the non-massive particles? counter intuitive result, more massive cluster has a dimmer stream! 

            By modeling globular clusters with the restricted three-body problem, we neglect mass loss due to internal dynamics. The internal dynamics of globular clusters involve a rich variety of physical processes that not only drive their internal evolution but also contribute to stellar escape through mechanisms other than tides. Many of these processes are reviewed in \citet{2003gmbp.book.....H} and have been the subject of decades of study (citation dump). Interestingly, many macrophysical behaviors emerge from microscopic interactions, such as repeated two-body encounters or three-body interactions. In this section, I focus on how these internal processes can affect the morphology and properties of the stellar streams that globular clusters produce.

            To begin, recent works by \citet{2023MNRAS.518.4249G,2024MNRAS.528.5189G} performed a study similar to our Chapter~4. While we investigated the mass loss of the globular cluster catalog due to tidal stripping, they examined the cumulative mass loss induced by three-body interactions within globular cluster cores. An example of this mass ejection distribution is shown in Fig.~\ref{fig:grondin_et_al_core_spray}. Indeed, globular clusters host many binary systems, and their properties can significantly influence cluster dynamics (citation dump). As \citet{2024MNRAS.528.5189G} describe, a third star can perturb a binary, leading to the ejection of one star with significant kinetic energy. Such velocities can exceed the cluster escape velocity and, in some cases, be high enough to completely erase the effective potential barrier (Fig.~\ref{fig:CR3BP_forbidden_region}) — typically controlling escape through the Lagrange points — no longer applies. The result is a population of stars that escape in a more isotropic and diffuse manner than those lost through tidal stripping alone.
            ...
            Introducing stellar masses into the system unlocks a suite of dynamical effects. One well-known example is \textit{dynamical friction} \citep{1943ApJ....97..255C,1943ApJ....97..263C,1943ApJ....98...54C}, which describes how massive bodies lose orbital energy through interactions with less massive ones, causing them to sink toward the center of the cluster. This process is the basis for \textit{mass segregation}, in which massive stars concentrate toward the core. The underlying statistical tendency is toward energy equipartition, where the average kinetic energy per particle is constant across the system. Since $E_k \propto m \langle v^2 \rangle$, this implies $\langle v^2 \rangle \propto 1/m$. However, in real clusters, relaxation times can be long, and equipartition may remain incomplete over the cluster lifetime \citep{2016MNRAS.458.3644B}. Moreover, since tides preferentially remove the stars on the most weakly bound (i.e., high-energy) orbits, they tend to strip low-mass stars first.

            When we consider not just that stars have mass, but that they are actually stars — luminous, gaseous bodies in hydrostatic equilibrium — an even more complex picture emerges. Stars vary in mass, age, and composition, which affect their luminosity and detectability. \citet{2018MNRAS.474.2479B} explored how internal cluster dynamics shape the escape of stars with different luminosities. They arrived at a seemingly paradoxical result: more massive clusters can produce \textit{less detectable} stellar streams. The reason is twofold: more massive clusters harbor more low-mass stars, which are intrinsically dimmer, and these same stars are preferentially stripped due to mass segregation. The resulting streams, though potentially more massive, are fainter and harder to detect (see Fig.~\ref{fig:balbinot-2018}).

            Adding another layer, stars themselves \textit{evolve} over time. \citet{2018MNRAS.474.2479B} incorporated stellar evolution in their simulations, capturing its early effects on cluster evolution. The most significant contribution occurs during the first $\sim$1 Gyr, when massive stars end their lives as supernovae. These events eject mass from the cluster, shallowing its potential well.  
