...
\section{The Ignored Physics} \label{sec:ignoredphysics}
    ...
    \subsection{Globular Cluster Internal Dynamics} \label{sec:collisionalDynamics}
        ...
        \subsubsection{Two-body Relaxation Time: a derivation and example}
            ...
            Now consider not just one fly-by, but a sequence of them. A real galaxy has a radially decreasing density, so a star never sees a symmetric distribution unless it's exactly at the center — but we are interested in \textbf{fluctuations} from the mean field, not the mean field itself. That is, if $\Phi_\mathrm{MW} = \Phi_\mathrm{mean}(\vec{r}) + \Phi_\mathrm{stochastic}(\vec{r})$, we are isolating the stochastic part.
            ...
            \begin{figure}
                ...
                \caption{
                    Demonstration of two-body relaxation. A star travels through a medium with uniform number density ($n = 500/\pi R^3$). Six independent trajectories are shown, each sampling a different realization of the same underlying number density. Particles are generated on-the-fly as the star moves. The right panel shows the cumulative squared change in velocity. The vertical line indicates the theoretical two-body relaxation time\footnote{The two-body relaxation experiment is purely illustrative. Despite multiple attempts, I was unable to make the numerical results consistent with the theoretical prediction. Not only is the relaxation process significantly slower than expected, but the discrepancy also worsens with increasing particle number. I rewrote the script several times but was unable to identify the issue. Nonetheless, the main takeaway remains valid: gravitational two-body encounters induce a random walk in velocity space.}.
                }
                \label{fig:twoBodyRelaxation}
            \end{figure}

            To perform this experiment, I normalize units such that $G = M = R = t_\mathrm{cross} = 1$, giving $v_0 = 1$ and $m = M/N_p$. For Fig.~\ref{fig:twoBodyRelaxation}, $N_p = 500$, so the number density is $500/\pi$. 
        ...
        \subsubsection{Beyond Two-Body Relaxation}
            ...
            In \citet{2024MNRAS.528.5189G}, as in the present thesis, the authors investigated mass loss from the Milky Way globular clusters. However, rather than focusing on external tidal forces, they examined internal three-body encounters occurring in the cluster cores. One of their striking results is reproduced in Fig.~\ref{fig:grondin_et_al_core_spray}, which clearly shows that many stars are ejected onto orbits that no longer align with the parent cluster—effectively spreading across the entire sky.
            ...
            In essence, the amount of energy lost from the sum of all two body interactions on a body can be estimated  \textit{stars} and not just point masses. Indeed, stars are super-hot gaseous spheres sustaining themselves from thermo-nuclear reactions and they \textit{evolve} in time \citep{2013sse..book.....K}. Lynden bell? core collapse??
