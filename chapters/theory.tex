...
\section{The Ignored Physics} \label{sec:ignoredphysics}
    ...
    \subsection{Globular Cluster Internal Dynamics} \label{sec:collisionalDynamics}
        ...
        \subsubsection{Beyond Two-Body Relaxtion}
            The granularity of a the medium slowly deflecting the trajectory of a given orbit isn't the thing that happens within a globular cluster. Indeed, this is a vast subject. Simplifying the problem into a couple key ingredients is the subject of the authoratative text \citet{2003gmbp.book.....H}. In essence, with two-body relaxation we only considered stochastic variations off of a mean field. However, if one wishes to understand, then you need to quantify all of the different type of \textit{micro-interactions}. \textit{insert good description here}

            \citet{2023MNRAS.518.4249G} \& \citet{2024MNRAS.528.5189G} investigated three-body encounters within globular clusters. The idea being that a solo star can take energy from a binary system, and take enough energy such that it gets ejected from the cluster. This is a completely different mechanism from the tidal stripping investigated in this thesis. Indeed, the tidal field gently gives particles energy so that they escape from the lagrange points. However, the three body encounters can provide \textit{alot} of energy to the star, the \textit{forbidden regions} of Fig.~\ref{fig:CR3BP_forbidden_region} disappear.
            
            \citet{2024MNRAS.528.5189G}, like the work presented in this thesis, studied the mass loss of the globular cluster catalog but instead of using tidal forces, they studied three body encounters within the cores of globular clusters. One of their results is presented in Fig.~\ref{fig:grondin_et_al_core_spray}. Indeed, here it is clear that many of the stars are not retaining the same orbit as the host cluter are they completely fill the sky. 
            \begin{figure}
                \includegraphics[width=\linewidth]{images/grondin_et_al_core_spray.jpeg}
                \caption{From \citep{2024MNRAS.528.5189G}}
