...
\section{The Implicit Physics}
    ...
    \subsection{The Distribution of Escaped Stars}
        ...
        \textit{Phase mixing} is a direct consequence of Liouville's theorem, which itself follows from the collisionless Boltzmann equation. Liouville's theorem states that the infinitesimal volume element in phase-space, $d\matbf{p}d\mathbf{q}$, is preserved under Hamiltonian evolution. In other words, as a system evolves in time, the phase-space density \( f(\mathbf{q}, \mathbf{p}, t) \) is conserved along particle trajectories. Since the number of particles is conserved, and the phase-space volume they occupy does not change.

        Insted of considering a infinitesimal volume of phase-space about some coordinate, consider a little cloud of many initial conditions dispersed about this coordinate. The total phase-space volume must be conserved, but the exact configuration does need to remain the same. Indeed,
        
        However, a phase-space volume is six dimensional. The particles may spread out spatial as long as the density decreases in a manner that preserves the original total phase-space volume. In physical space, this corresponds to a decrease in local density as the particles become more dispersed—this is the essence of phase mixing.
