...
\section{The Explicit Physics}
    ...
    \subsection{The Gravitational Field} \label{subsec:gravfield}
        ...
        Note that many of the aforementioned works rely on fitting the data to kinematic information, i.e. comparing the models prediction of the velocity dispersion of a statistically significant of stars to that of a chosen data set. This is because, astrometric measurements of accelerations is still in a very early phase. Detecting a change in angular position via the parallax is so difficult that people used to refer to the stars as \textit{fixed-stars} \citep{1981unht.book.....K}. Then, the measurement must be made on a yearly cadence with high precision in order to detect a star's movement on the celestial sphere, which is exactly what Gaia is going. \textbf{whose uncertainties are lower? Proper motiosn or Radial velocities? is it possible to measure the accelerations using RV?}. However, it's not to say that accelerations have never been detected. \citet{2018ApJS..239...31B,2021ApJS..254...42B} created a cross-catalog between Hipparcos and Gaia Data Release 2 \citep{2018A&A...616A...1G} and then EDR3. By doing so, a catalog of stars has over a 25-year baseline of observations, which is long enough that for many stars an acceleration can be detected. However, the Hipparcos catalog is still limited to 100,000 stars. Additionally, the magnitude of the acceleration of a galactic field star is still too low. Currently, this catalog is being leverage to systems with unusually high accelerations, which are indicative of binary star systems or stellar systems with massive plants \citep{2023MNRAS.521.5232W,2025AJ....170...52G}, and not to measure the accelerations due to the galactic gravitational field. 

        However, as motivated by the goal of this thesis, stellar streams are incredible interesting for this reason. Since the stars are on more or less the same orbit but only shifted in phase, they encode the gravitational field of the Milky Way. \citet{2024ApJ...967...89I} is the first work to use the stellar streams to fit a gravitational potential model of the Milky Way. Even in this, \citet{2024ApJ...967...89I} fixed some parameters of the model, for instance they employed the same bulge model as \citet{2017MNRAS.465...76M}.

        We choose the model of \citet{2017A&A...598A..66P}, which built upon \citet{1991RMxAA..22..255A}. This model was made by our group and made availbe to me at the onset of the project. This model was constructed to fit similar observational data as \citep{2015ApJS..216...29B}, such as different rotation curves and kinematic properties of stars within the local solar volume. \citep{2017A&A...598A..66P} provides two different models, one with bulge and one without. Below, I will describe the second model of \citep{2017A&A...598A..66P}, since we employed in both chapter~4 and chapter~5.  

        The second model of \citep{2017A&A...598A..66P} models the galaxy with a halo, a thin and thick disk. One advantage of this model is that each potential for each component is analytic. For instance, the potential of the Miyamoto-Nagai \citep{1975PASJ...27..533M}, is fully analytical. Although it provides a density profile with a polynomial decay rather than exponential decay, it is still a fair model for the galactic disks. The potential has the form of:
        \begin{equation}
            \Phi(R, z \mid M, a, b) = -\frac{G M}{\sqrt{R^2 + \left(a + \sqrt{z^2 + b^2}\right)^2}},
        \end{equation}
        where $M$ is the total mass of the disk, $a$ is the radial scale length, and $b$ is the vertical scale height. This form smoothly interpolates between a flattened disk and a spherical distribution depending on the values of $a$ and $b$. \citet{2017MNRAS.465...76M} also used this model for their galactic disk.
        
        However, it is worth mentioning that, in general, it is known the galaxy's have exponential decaying disks: 
        \begin{equation}
            \rho(R,z) = \rho_0 \mathrm{exp}\left(-\frac{R}{R_d}-\frac{|z|}{z_d}\right),
        \end{equation}
        where the density, $\rho$ is a function of the cylindrical radius $R$, vertical height, $z$, and has three parameters: a characteristic density, scale radius, and scale height: $\rho_0$, $R_d$, $z_d$. The difficulty with this model is that it cannot be easily integrated to obtain the potential since the $R$ and $z$ coordinates are not separaetable. Often times, a basis function expansion must be employed to approximate the potential when computing the forces. \citet{2017MNRAS.465...76M} and \citet{2024ApJ...967...89I} both employed this model to describe their galactic disk. 


        Regarding the halo, in \citet{2017A&A...598A..66P} does not create a different halo model for the dark matter and the stellar component. Instead they use a single halo model to describe the total gravitational potential from these components, and inherrit the same halo model used in \citet{1991RMxAA..22..255A}, which was originally proposed in \citep{1986RMxAA..13..137A}. This model was constructed such that at distances greater than some scale radius, the enclosed mass would grow linearly with distance. This leads to the desireable property of a flat rotation cruve. This model is given by:
        \begin{equation} 
        ...
        In general, a good halo model should to reproduce the observed flat rotation curve at large radii. Another class of families does this, which is worth mentioning, which is known as double power-law models:
        \begin{equation}
        ...
        Two Miyamoto-Nagai disks plus the Martos halo compose the second potential model presented in Pouliasis2017pii and is presented in Fig.~\ref{fig:figure_pouliasis2017pii_potential}. More visualizations of this potential can be seen in their work. 
        ...
        In the limit $a \to 0$, the Miyamoto-Nagai potential reduces to the Plummer potential:
        \begin{equation}
