...
\section{The Explicit Physics}
    ...
    \subsection{Generating a Globular Cluster}\label{subsec:initialconditions}
        ...
        \subsubsection{Sampling the Initial Conditions}
            ...
            The probability distribution for an isotropic Plummer sphere depends on the energy only, which in turn depends on the radial distance and the speed of a particle: $f(r,v) \propto (-E)^{7/2}$. If we marginalize over the velocities, we obtain the one dimensional density profile. The CDF of said distribution is the enclosed mass as a function of radius. For a Plummer sphere, this is: 
            \begin{equation}
            ...
            A demonstration of applying the inverse transform sampling to this CDF is shown in Fig.~\ref{fig:inverse_transform_sampling_distances}. the intersections between the uniform random numbers and the CDF are found. The intersection then provide the radial distances.
            \begin{figure}
                \includegraphics[width=\linewidth]{images/inverse_transform_sampling_distances.png}
                \caption{Inverse transform sampling of the mass distribution function of a Plummer sphere. 1000 uniform random numbers between [0,1] were generated and intersected with the normalized enclosed mass distribution. The intersection points then determine the corresponding sampled position, which is highlighted in the top panel. The top panel only shows 100 samplings to not over crow the plot. The bottom plot shows the resulting distribution of radii. }
                \label{fig:inverse_transform_sampling_distances}
            \end{figure}

            A joint distribution function is the product of a conditional probability DF and a marginalized distribution function: $f(r,v)=f(v|r)f(r)$. The number of particles at a given speed can be found by integrating over the DF and then redifferentiating with respect to the speed:
            \begin{equation}
                \frac{dN}{dv} = \frac{d}{dv}\int f(\textbf{v}|r) (v\sin(\theta)d\phi) (vd\theta)(dv) = 4\pi v^2 f(v|r).
            \end{equation}
            Note that the angles here are not in physical space but rather velocity space. Lastly, to construct the CDF of this DF, we must know what the proper limits are. The speeds available to this particle range from zero to the escape speed, which is $v_{\mathrm{esc}} = \sqrt{2\Phi(r)}$. Thus, the CDF is: 
            \begin{equation}
                \mathcal{F}(v) = C \int_0^{v<v_\mathrm{esc}} 4\pi v^2 f(v|r) dv
            \end{equation}
            where $C$ is some normalization constant. I do not present this normalization constant analytically. Computationally, I ignore the scaling constants and renormalize everything  a-posteriori. The CDF may easily be renormalized by the final value of the unnormalized CDF: $C=1/\mathcal{F}(v_\mathrm{esc})$.    
            
            Sampling the speeds is shown in Fig.~\ref{fig:inverse_transform_sampling_velocities}. Since we are sampling a conditional probability distribution, each particle has it's own CDF given it's position, contrary to Fig.~\ref{fig:inverse_transform_sampling_distances} where all distances are sampled from the same marginalized DF. Note that for the clusters that are farther from the center, there are less possible speeds, since it could be come unbound. 
            \begin{figure}
                \includegraphics[width=\linewidth]{images/inverse_transform_sampling_velocities.png}
                \caption{Inverse transform sampling of the speed of a particle given it's position. Each particle has it's own CDF which is only sampled once. The figure shows a Plummer sphere with N=1000. The top panel only shows 25 samplings in order to not clutter the plot, each of which is color coaded by the sampled distance. The random number and it's intersection with the plot is indicated by the marker of the same color as the curve. The bottom panel shows the distribution of speeds. The maximum value is $\sqrt{2\frac{GM}{r_c}}$, which would be the escape velocity at the center of the cluster.}
                \label{fig:inverse_transform_sampling_velocities}
            \end{figure}
            Note that this process could be inverted. The distribution function could be marginalized over the positions first to find one CDF for the speeds. Then, given a speed, we would find an individual CDF per particle to obtain a sampling of it's distance from the cluster's center. While valid, this is not favored since it's less intuitive. Indeed, many models are motivated by there density profiles which are intuitive and observed quantities, which in turn serves as a natural starting point for the inverse transform sampling. 
