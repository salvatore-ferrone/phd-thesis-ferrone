...
\section{The Explicit Physics}
    ...
    \subsection{The Gravitational Field} \label{subsec:gravfield}
        ...
        \begin{itemize}
            ...
            \item Spiral arms.
        \end{itemize}
        In addition, smaller-scale structures such as globular clusters, giant molecular clouds, and open clusters can locally perturb the field. Even external sources, such as the Large and Small Magellanic Clouds, play a role by inducing measurable tidal effects on the Galaxy \citep[see, for example][]{2021MNRAS.501.2279V,2022ApJ...939....2A}.\paola{anche ne caso della prima delle due citazioni, appaiono i nomi di Eugene e Vasily.}
        ...
        \begin{figure}
            ...
            \caption[The Gaia spatial selection function for spectroscopic observations]{The Gaia spatial selection function for spectroscopic observations. Figure 5 from \citet{2023A&A...674A..38G}.}
        ...
        In Chapter~4, and in some preliminary results in the Chapter~6, we use a bar for the interior region of the galaxy. The model was first presented in \citet{1992ApJ...397...44L}, who was motivated to create a simple analytical model with interpretable parameters. The model was created by first describing the bar as a ``needle'', with constant density along the needle and zero elsewhere. The authors then convolve this with a plummer sphere, ensuring in the limit that the bar's lenght is much smaller than it's thickness, a plummer potential is recovered, in the limit of the length being much larger than the thickness, a needle is recovered. The authors continued, to create a tri-axial bar model. Instead of convolving with a Plummer sphere, they connvolved the needle lying along the x-axis with a Miyamoto-Nagai disk flattened along the z-axis. The potential is given by: 
        \begin{equation}
            \Phi(x,y,z|M,a,b,c)=\frac{GM}{2a}\ln\left( \frac{x-a+T_{-}}{x+a+T_{+}}   \right),
        \end{equation}
        with $T_{\pm}=\left[ (a\pm x)^2+ y^2 +  (b+ \sqrt{c^2+z^2})^2 \right]^{1/2}$. With this parameterization, $M$ is the total mass of the bar, $a$ is the bar's length. $b$ and $c$ control if the bar is a flat needle or an axis-symmetric prolate bar. If $c/b\rightarrow \infty$, the bar becomes prolate. If $c/b \rightarrow 0$, then a flat-planar bar is recovered. $a/\mathrm{max}(b,c)$ sets the elongation of the bar, where $a/\mathrm{max}(b,c)\rightarrow \infty$ recovers the needle potential. 

        
        Sometimes, instead of including One of the models of \citet{2017A&A...598A..66P} includes a buldge and models it using a Plummer sphere \citep{1911MNRAS..71..460P}. This model has two parameters—the total mass and a scale length—and takes the form
        ...
        \paola{Salvatore, trovo che qui almeno un accenno al potenziale di barra vada fatto. Citi dopo a varie riprese che in alcuni casi il potenziale non e' axisimmetrico, e se il lettore vuole sapere da dove viene l'asimmetria e che potenziale di barra hai scelto, non trova l'info, mentre dovrebbe essere qui. Quando menzioni il potenziale triassiale di Long \& Murali che hai usato, puoi anche ricordare che, interestingly, per un'opportuna scelta di parametri, si riduce al potenziale di Myamoto-Nagai, che a sua volta, come hai ricordato, e' legato al potenziale di Plummer.}
...
\section{The Implicit Physics}
    ...
    \subsection{Tidal Forces}\label{sec:tidalForce}
        ...
        \end{equation}
        where \(\Delta \vec{x} = \vec{x}_s - \vec{x}_c\), and \(\mathcal{D}^2 \Phi_g\) is the Hessian matrix of second derivatives of the potential: \(\partial^2 \Phi/\partial x_i \partial x_j\). \paola{forse dire anche che $\vec{x}_c$ e' la posizione del baricentro del primary body?}

        The second expression can be derived by linearizing the gravitational force in a non-inertial frame co-moving with the secondary \paola{nelle formule che seguono $\Phi_c, \vec{F}_s, \vec{F}_c$ non mi sembrano definite}. Let us write Newton's second law for the star-particle and the secondary in an inertial frame:
        \begin{eqnarray}
        ...
        \subsubsection{The Moon}
            ...
            \begin{equation}
                ...
                \end{matrix}\right),
            \end{equation}
            which has eigenvalues $2\frac{GM}{r^3}$, $-\frac{GM}{r^3}$, and $-\frac{GM}{r^3}$, with corresponding eigenvectors \paola{ricordiamoci di riverificare alcuni degli autovettori qui sotto}:
            \begin{equation}
...
\paola{dobbiamo discutere dei videos :))}
            \begin{verbatim}
        ...
        \subsubsection{Tides from the disk}
            ...
            \begin{equation}
                ...
                \end{matrix}\right).
            \end{equation}             
            \paola{definisci $\beta'$ e $\beta''$} We immediately notice that, due to the cylindrical symmetry, the eigenvectors are not as simple as in the spherical case. As long as 
            \[
    ...
    \subsection{Massive objects colliding with stellar streams}
        Previous\paola{ly ?} we described how they\paola{stars?} escape the globular clusters. Then we described how the escaped stars can redistribute themselves in the galaxy following a nice evolutionary flow in the gravitational field from normal hamiltonian evolution. However, other things can happen to the stream. Notably, other massive objects in the Galactic field can fly-by and perturb the orbits of some of these stars. Here, I would ike to summarize this. 

        Collisions are essential in this thesis for two reasons. Firstly\paola{you say "Firstly" but I don't see a second point ..}, one of the main research questions is how the passage of a subhalo perturbs a stellar stream. This interaction can be treated as a gravitational collision.
        ...
        \subsubsection{Gap formation and evolution}
            ...
            \begin{enumerate}
                ...
                \item \textbf{Caustic phase} -- Eventually, stars bunch up at different locations at the edges of the gap, creating sharp overdensity features (``caustics''). Additionally, the gap's growth to\paola{o?} is slowed to a sublinear rate, scaling roughly as $\propto \sqrt{t}$.
            \end{enumerate}


            The authors make an important realization. Even in a scenario where the Galactic potential and the stream's orbit are known, with just the gap's density profile there are still seven parameters that describe its shape \paola{questa frase non mi e' completamente chiara, in particolare 'with just the gap's density profile ..'. forse vuoi dire o puoi dire:  Even in a scenario where the Galactic potential and the stream's orbit are known, there are still seven parameters that describe the shape of the gap's density profile..  } : $M,r_s,b,w_\perp,w_\parallel,\alpha,t$. The authors state that the gap's profile provides two more piece of information and, with a constraint on the perturber's density, we would still be left with four degrees of freedom. The problem is thus underdetermined and it is thus impossible to uniquely determine the properties of the perturber. A large statistical sample of gaps would be required to constrain a population of dark matter subhalos. With the ever increasing data quality, perhaps this could be possible someday.  

            While powerful, the authors describe how the limiting assumptions impact the interpretations of this\paola{their?} work. First, the framework is limited to circular orbits which poses a significant constraint especially for globular clusters which often have inclined and eccentric trajectories. Indeed, as they discussed in their work, streams do not have uniform densities. Stars that populate the streams come from tidally disrupting stellar systems. As they leave the cluster and enter the stream, they come with a range of energies and angular momenta that in essence eliminate any caustic features. 
            ...
            \end{figure}
            \textit{Note:} I would love to explore this more rigorously in the context of time-dependent or eccentric mass loss, where the source term becomes periodic. I did begin studying such a model using a time-dependent Gaussian source, but at some point the ``simple approach'' becomes as complex as full numerical simulations. \paola{tutta questa parte mi sembra ottima. Quello che forse manca, qui o nelle conclusioni (che devo ancora leggere) e' lo studio che hai fatto per il paper 2, sulla visibilita' dei gaps in streams caratterizzati da popolazioni di stelle perse a tempi diversi, con velocita' medie diverse. Credo (ma devo finire di leggere tutto) che a questo lavoro non accenni nella tesi. Non so se e' a questo che pensi quando parli qui sopra di 'eccentric mass loss'.. }
...
\section{The Ignored Physics} \label{sec:ignoredphysics}
    ...
    \subsection{Globular Cluster Internal Dynamics} \label{sec:collisionalDynamics}
        ...
        \subsubsection{Two-body Relaxation Time: a derivation and example} \label{sec:twoBodyRelaxation}
            ...
            \end{figure}
            To perform this experiment, I normalize units such that $G = M = R = t_\mathrm{cross} = 1$, giving $v_0 = 1$ and $m = M/N_p$. For Fig.~\ref{fig:twoBodyRelaxation}\footnote{The two-body relaxation experiment is purely illustrative. Despite multiple attempts, I was unable to make the numerical results consistent with the theoretical prediction. Not only is the relaxation process significantly slower than expected, but the discrepancy also worsens with increasing particle number. I rewrote the script several times but was unable to identify the issue. Nonetheless, the main takeaway remains valid: gravitational two-body encounters induce a random walk in velocity space. \paola{non abbiamo avuto tempo per discutere da dove venisse questa discrepanza e cosa hai confrontato con cosa .. forse c'e' qualcosa di evidente per risolvere il pb, ma non credo avere gli elementi (= lo script) e il tempo per aiutarti ..}}, $N_p = 500$, so the number density is $500/\pi$. 
            ...
            \]
            assuming an average stellar mass of 0.8 $M_\odot$ \paola{questo valore e' circa il valore della massa al turn-off, ma non il valore medio della massa di una stella in un ammasso, che e' circa 0.4-0.5Msun (vedi ad es calcoletto Sect 2.1.1., Baumgardt 1998 (essenzialmente usi una mass function, limite minimo e massimo per le masse e ottieni la media)} [citation needed]. With this, only a handful of clusters have relaxation times greater than the 5 Gyr integration time used in our simulations. These include: 
