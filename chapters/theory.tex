In this chapter, I summarize the theoretical background necessary to understand how we model the disruption of globular clusters and the formation of stellar streams. My goals are fourfold:
\begin{enumerate}
...
\section{The Explicit Physics}
    ...
    The scaling of computation time is often analyzed using Big-O notation. For direct $N$-body simulations, the computational cost scales as $\mathcal{O}(N^2)$, since the gravitational force on each particle must be computed from every other particle. For the Milky Way, with approximately $10^{11}$ stars, this implies on the order of $5 \times 10^{21}$ pairwise computations per time step.

    Let us consider the practicality of performing such a computation. FLOPS (floating point operations per second) is a common metric for computational speed. Frontier, one of the most powerful supercomputers currently available, operates at approximately one exaflop, or $10^{18}$ FLOPS \citep{atchley2023frontier}. Assuming—conservatively—that each pairwise computation requires a single FLOP, a full $N$-body computation of the Galaxy at one time step would still require several hours. While this may seem manageable, a single time step is insufficient for modeling the long-term evolution of the system, which would require millions of steps. Dedicating an entire exascale machine to such a task is therefore a considerable demand.

    Moreover, the cost of power is non-negligible. At an estimated rate of $\sim 0.10~\mathrm{USD}/\mathrm{kWh}$ \citep{table20245}, and with Frontier consuming roughly 21 MW, a single 6-hour computation would cost approximately 12,600 USD. Running the system for a full day would amount to about 50,000 USD, or ~42,000 EUR at the exchange rate of 1 EUR = 1.17 USD on 25 July 2025 (European Central Bank). This is nearly twice the annual salary of a PhD student in France (24,000 EUR) \citep{MESR_financement_doctoral}.

    As a humorous aside, consider attempting this computation on a personal research laptop, such as a MacBook Air with an M2 processor. Its estimated speed of $\sim$0.1 TFLOPS (i.e., $10^{11}$ FLOPS) \citep{hubner2025apple} is approximately 10 million times slower than Frontier. A single time step would thus take over 1,500 years.

    Clearly, we require a more tractable modeling approach for studying the evolution of globular clusters and the formation of stellar streams. Fortunately, due to the long two-body relaxation time of the Galaxy, it is well justified to approximate the Galactic potential as a smooth background density field rather than as a collection of point masses. This argument, presented in Section 1.2 of \citet{2008gady.book.....B}, shows that the inaccuracy of such a model becomes significant only on timescales far exceeding the age of the Universe. We revisit this derivation in Section~XXX of this thesis.

    The two-body relaxation time is inversely proportional to the local stellar density. In globular clusters, where stars are densely packed into a relatively small volume, stellar encounters become significant over the system's lifetime. According to Chapter 5.1 of \citet{bovy_inprep}, the relaxation timescale within globular clusters is typically $\sim 1$~Gyr. This is shorter than the age of most clusters, yet significantly longer than the dynamical timescale associated with individual stellar orbits.

    To be more specific, globular clusters typically span a few parsecs, with internal velocity dispersions of a few km/s \citep{2018MNRAS.478.1520B}. The corresponding crossing time—the characteristic time for a star to traverse the system—is roughly 1 Myr. Additionally, globular clusters are old systems, with ages ranging from several billion to over ten billion years \citep{2013ApJ...775..134V}.
    ...
    This formulation dramatically simplifies the dynamics: the full $N$-body problem is approximated by \(N\) independent realizations of the restricted three-body problem. This reduction allows us to efficiently simulate the formation and evolution of tidal features without incurring the computational cost of tracking all mutual interactions among stars.
