    ...
    \subsection{The tidal tensor}
        ...
        \subsubsection*{Tides in the Galaxy}
            ...
            \begin{equation}
                ...
                \end{matrix}\right).
            \end{equation}             
            We immediately notice that, due to the cylindrical symmetry, the eigenvectors are not as simple as in the spherical case. As long as $\beta'^2 + \beta \beta'' -\frac{3\left(\beta\beta'\right)^2}{D^2} \neq 1-\frac{3z^2}{D^2}$: (1) the stretching axis is no longer parallel to the position vector and (2) compression axes do not necessarily span the same plane, but may be fixed and different in magnitude. The exact orientation of all three eigenvectors depends on the cluster's position in the galaxy. Note that if $z=0$, we recover the stretching eigenvector being parallel to the position vector, although the orientations of the eigen vector are fixed as the compression axes are not the same in magnitude. If, we $b=0$, then we recover spherical symmetry and the compression eigenvalues are the same in magnitude.

            \begin{verbatim}
            VIDEO: tidal_deformation_ellipsoid.mp4
            \end{verbatim}

            I have prepared Fig.s~\ref{fig:miyamoto_disc_shocks_circular_inclined_orbit}-\ref{fig:miyamoto_disc_shocks_weak_shocks} to demonstrate a variety of the tidal forces in a variety of different orbits within a Miyamoto-Nagai disk potential. Fig.~\ref{fig:miyamoto_disc_shocks_circular_inclined_orbit} presents a cluster on an inclined yet non-eccentric orbit. While Fig.~\ref{fig:miyamoto_disc_shocks_planar_eccentric_orbit} presents the opposite, an eccentric orbit with no inclination with respect to the disk. The former has disc-shocks while the later only has increased magnitude due to the pericenter passages. 

            Fig.~\ref{fig:miyamoto_disc_shocks_responant_R_z} presents an orbit where the vertial and radial orbital periods are nearly resonant, leading to two disk shocks about every pericenter passage. Fig.~\ref{fig:miyamoto_disc_shocks_big_apocenter} presents a cluster on an orbit with a large apo-center resulting very weak tidal forces. Lastly, Fig.~\ref{fig:miyamoto_disc_shocks_weak_shocks} presents a cluster in a potential wit ha very thick disk. In this case, the disk shocks occur over a longer period of time and are less dramatic.
