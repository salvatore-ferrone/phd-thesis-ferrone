...
\section{The Ignored Physics} \label{sec:ignoredphysics}
    ...
    \subsection{Collisional dynamics} \label{sec:collisionalDynamics}
        As shown in multiple sources, it is appropraite to consider the orbits of stars within the disk or the galaxy as being collisionless. However, this is unsatisfactory for globular clusters. I want to show a little bit of that here. 

        I don't want to represent the whole derivation but present a little numerical experiment I conducted to show what two-body relaxation is as it is presented. 

        Two body relaxation aims at quantifying the innaccurary of the mean-field approximation. It formualtes the question in this manner, how long does it take for the granularity of the medium to cause my trajecotry to significantly deviate from it's trajectory if it were in a perfectly homogeneous medium? The metric is thus finding the amount time such that $\delta v / v_0 \approx 1$. 

        These types of calculations really abstract the motion of a particle from that of an orbit in the galaxy. The exact orbit of the star is completely neglected. Instead, we consider uniform motion. Then, we consider the motion of a star in a medium made of discrete particles. If the mass is distributed onto many particles, then each one is smaller, and there are more of them, and we better approximate the mean field. However, if there are less of them, then each one is significant and can perturb the motion of our particle in a significant way. 

        For instance, the consider the vector quanity of the change in velocity due to an impact within the impulse approximation. It is:
        \begin{equation}\label{eq:delta_v_vec_impulse_approx}
            \delta \mathbf{v} = \frac{Gm}{bv}\begin{bmatrix} 0 \\ \cos\left(\theta\right) \\ \sin\left(\theta\right) \end{bmatrix},
        \end{equation}
        Imagine now, that this particle is flying through the galaxy. Now, instead of considering just one fly-by, we want to consider all fly-bys that could happen. It's too difficult to consider a real galaxy. Since the stellar density decays with distance, and if we are never in the center, then one side should have more stars than the other. However, that would be considering the mean-field and we want to completely abstract the stochastic fluctuations from the mean field. So if the gravitational field of the Milky Way can be considered: $\Phi_\mathrm{MW} = \Phi_\mathrm{mean}\left(\vec{r}\right) + \Phi_\mathrm{stochastic}\left(\vec{r}\right)$, then we only need consider the stochastic fluctuations and not the mean field that contains the over all density trends. 

        So, to really simplify this problem, we can consider the galaxy to be a cylinder of finite width but infinite length, with a number density $n=N_p/\pi R^3$, whose length and radius have the same length. As the particle passes through the medium, using the impulse approximation, we can track how the velocity changes. These impulses kick the particle to the left or the right but never give or take along the direction of motion, by definition within the impulse approximation. I present this little numerical experiment in Fig.~\ref{fig:twoBodyRelaxation}.
        \begin{figure}
            \includegraphics[]{images/twoBodyRelaxation.png}
            \caption{Demonstration of Two-Body Relaxation. A star traverses a medium of uniform number density. I present 5 different trajectories who sample their own media, of the same number density, yet each one is sampled stochastically. The number density is 1000/$\pi R^3$. The particles are generated as the star moves and therefore they can keep diverging. The graph on the right presents the cummulative change in velocity squared. The vertical presents the two theoretical relaxation time.}
            \label{fig:twoBodyRelaxation}
        \end{figure}
        Briefly, to perform the experiment in Fig.~\ref{fig:twoBodyRelaxation}, I normalize the problem to: $G=1$, $M=1$, $R=1, t_\mathrm{cross}=1$, which determines that $v_0=1$, $m=M/N_p$. For the example presented in Fig.~\ref{fig:twoBodyRelaxation}, there are 500 particles. The density of the medium is thus: $500/\pi$. I select a total integration period, and a time step $dt$. At each time step, the volume a disk is found by: $\pi R^2 v dt$ and is populated with a uniform sample of particles. The number of particles to use is found through Poisson statistics by stating that the expected number of particles within the disk is $\langle N_{p,\mathrm{disc}} \rangle = n \pi R^2 v dt$.  By using Eq.~\ref{eq:delta_v_vec_impulse_approx} and summed together to find the net change in velocity and position. This step is repeated at the next time step. 


        Theoretically, in this idealized scenario, it can be shown that, on average:
        \begin{equation}
            \langle \delta \mathbf{v}\rangle = \mathbf{0},
        \end{equation}
        which is unsurprisingly since the medium is uniformly distributed, and integrating sine or cosine over one full period is null. However, $\langle \delta v^2 \rangle$ is not zero. Given the assumptions above, we can find the rate of change of $\langle \delta v^2 \rangle$ per unit time: $\frac{d \langle \delta v^2 \rangle} {dt}$. Once this is found, one can find the two body relaxation time as: 
        \begin{equation}
            \tau_\mathrm{2-body} = \frac{v_0^2}{\frac{d \langle \delta v^2 \rangle}{dt}}.
        \end{equation}
        The last trick to this is that \textit{coloumb integral}, which is tricky since it integrates: $\int \frac{1}{R} dR$, which diverges, therefore one has to pick appropriate limits. The upper limit is the extent of the system, since there are no particles beyond this. The lower limit is tricker. We choose this to be the point at which the assumption breaks down. I choose it to be the distance at which, for a given speed, the particles would be bounded. This is generous since the impulse approximation would breakdown before this. However, it's just a quick esimate in the first place. 

        This gives the result that: 
        \begin{equation}
            \tau = t_\mathrm{cross}\frac{N}{8\ln\left(N/2\right)}.
        \end{equation}

        Let's use this result and apply it to the globular cluster catalog. First, let's pick the mediam crossing time per star within the cluster: $t_\mathrm{cross}=\sqrt{r_{1/2}^3/GM}$. Then, let's assume that the average mass per star is 0.8~$M_\odot$ (citation?). With these choices, this gives only a handful of clusters whose relaxation time is greater than the $5$ billion years used in our simulations. They are: NGC2419, NGC5139, Pal14, Pal15, FSR1758, Ter8,  SagittariusII. Also, the average relaxation time is 1.~gyr and the median time is 0.8~Gyr. 
